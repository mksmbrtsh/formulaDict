...
class VerticalBox extends Box {
    ...
    public VerticalBox(int index, String token) {
    	super(index, token);
    }

    public VerticalBox(int index, String token, Box b, float rest, int alignment) {
    	super(index, token);
        add(b);
        if (alignment == TeXConstants.ALIGN_CENTER) {
            StrutBox s = new StrutBox(index, token, 0, rest / 2, 0, 0);
            super.add(0, s);
            height += rest / 2;
            depth += rest / 2;
            super.add(s);
            ...
            depth += rest;
            super.add(new StrutBox(index, token, 0, rest, 0, 0));
        } else if (alignment == TeXConstants.ALIGN_BOTTOM) {
            height += rest;
            super.add(0, new StrutBox(index, token, 0, rest, 0, 0));
        }
    }
    ...
    public final void add(Box b, float interline) {
        if (children.size() >= 1) {
            add(new StrutBox(index, token, 0, interline, 0, 0));
        }
        add(b);
    }

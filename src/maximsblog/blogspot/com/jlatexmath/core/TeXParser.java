...
public class TeXParser {
					...
					if ("newcommand".equals(com) || "renewcommand".equals(com)) {
						...
						if (mac == null) {
							...
							try {
								...
								String grp = getGroup("\\begin{" + args[1]
										+ "}", "\\end{" + args[1] + "}");
			...
			while (pos < len) {
				...
				case ' ':
					...
					if (!ignoreWhiteSpace) {// We are in a mbox
						formula.add(new SpaceAtom(pos-1, " "));
						formula.add(new BreakMarkAtom(pos-1, " "));
						while (pos < len) {
							ch = parseString.charAt(pos);
							if (ch != ' ' || ch != '\t' || ch != '\r')
								break;
				...
				case DOLLAR:
					...
					if (!ignoreWhiteSpace) {// We are in a mbox
						...
						String g = getDollarGroup(DOLLAR);
						formula.add(new MathAtom(pos-1, g, new TeXFormula(this,
								g, false).root, style));
						if (doubleDollar) {
							if (parseString.charAt(pos) == DOLLAR) {
								pos++;
				...
				case SUB_SCRIPT:
					if (ignoreWhiteSpace) {
						...
						formula.add(new UnderscoreAtom(pos, SUB_SCRIPT + ""));
						pos++;
					}
					break;
				case '&':
				...
				case PRIME:
					if (ignoreWhiteSpace) {
						formula.add(new CumulativeScriptsAtom(pos, PRIME +"", getLastAtom(),
				...
				case BACKPRIME:
					if (ignoreWhiteSpace) {
						formula.add(new CumulativeScriptsAtom(pos, BACKPRIME +"", getLastAtom(),
				...
				case DQUOTE:
					if (ignoreWhiteSpace) {
						formula.add(new CumulativeScriptsAtom(pos, DQUOTE +"",getLastAtom(),
								null, SymbolAtom.get("prime")));
						formula.add(new CumulativeScriptsAtom(pos, DQUOTE +"",getLastAtom(),
								null, SymbolAtom.get("prime")));
					} else {
	...
	private Atom getScripts(char f) throws ParseException {
		...
		if (formula.root instanceof RowAtom) {
			...
			at = new PhantomAtom(pos, "",new CharAtom(pos, "M", 'M', "mathnormal"), false, true,
					true);
		} else {
			at = formula.root;
			formula.root = null;
		...
		if (at.getRightType() == TeXConstants.TYPE_BIG_OPERATOR)
			return new BigOperatorAtom(pos, f+"", at, first, second);
		else if (at instanceof OverUnderDelimiter) {
			if (((OverUnderDelimiter) at).isOver()) {
				if (second != null) {
					((OverUnderDelimiter) at).addScript(second);
					return new ScriptsAtom(pos, f+"",at, first, null);
				}
			} else if (first != null) {
				((OverUnderDelimiter) at).addScript(first);
				return new ScriptsAtom(pos, f+"",at, null, second);
			}
		}

		return new ScriptsAtom(pos, f+"", at, first, second);
	}
	...
	public String getGroup(String open, String close) throws ParseException {
		...
		if (group != 0) {
			...
			throw new ParseException("The token " + open
					+ " must be closed by " + close);
		}
	...
	public Atom getArgument() throws ParseException {
		...
		if (ch == L_GROUP) {
			...
			if (this.formula.root == null) {
				RowAtom at = new RowAtom(pos-1, ch + "");
				at.add(tf.root);
				return at;
			}
			return tf.root;
	...
	public Atom convertCharacter(char c, boolean oneChar) throws ParseException {
		...
		if (((c < '0' || c > '9') && (c < 'a' || c > 'z') && (c < 'A' || c > 'Z'))) {
			...
			if (!isLoading && !DefaultTeXFont.loadedAlphabets.contains(block)) {
				try {
					DefaultTeXFont
							.addAlphabet(DefaultTeXFont.registeredAlphabets
									.get(block));
			...
			if (symbolName == null
				...
				if (fontInfos != null) {
					if (oneChar) {
						return new JavaFontRenderingAtom(pos, Character.toString(c), Character.toString(c),
								fontInfos);
					}
					int start = pos++;
					int end = len - 1;
					...
					return new JavaFontRenderingAtom(start - 1, parseString.substring(
							start, end + 1), parseString.substring(
							start, end + 1), fontInfos);
				}
				...
				if (!isPartial) {
					...
					return new ColorAtom(pos, "", new RomanAtom(pos, "", new TeXFormula(
				...
				if (!ignoreWhiteSpace) {// we are in text mode
					if (TeXFormula.symbolTextMappings[c] != null) {
						SymbolAtom sa =  SymbolAtom.get(TeXFormula.symbolTextMappings[c])
								.setUnicode(c);
						sa.index = pos;
						sa.token = c+"";
						return sa;
				...
				try {
					SymbolAtom sa = SymbolAtom.get(symbolName);
					sa.index = pos;
					sa.token = c+"";
					return sa;
			...
			if (fontInfos != null) {
				...
				return new JavaFontRenderingAtom(start -1 , parseString.substring(start,
						end + 1), parseString.substring(start,
						end + 1), fontInfos);
			}
			return new CharAtom(pos, c + "", c, formula.textStyle);
		}
	}
	...
	private Atom processEscape() throws ParseException {
		...
		if (!isPartial) {
			...
			return new ColorAtom(pos, "", new RomanAtom(pos, "", new TeXFormula("\\backslash "
					+ command).root), null, Color.RED);
		}
	}

...
public class UnderOverArrowAtom extends Atom {
	...
	public UnderOverArrowAtom(int index, String token, Atom base, boolean left, boolean over) {
		super(index, token);
		this.base = base;
	...
	public UnderOverArrowAtom(int index, String token, Atom base, boolean over) {
		super(index, token);
		this.base = base;
		this.over = over;
		this.dble = true;
	...
	public Box createBox(TeXEnvironment env) {
		...
		Box b = base != null ? base.createBox(env) : new StrutBox(index, token, 0, 0, 0, 0);
		float sep = new SpaceAtom(index, token, TeXConstants.UNIT_POINT, 1f, 0, 0).createBox(
				env).getWidth();
		Box arrow;
		...
		if (over) {
			vb.add(arrow);
			vb.add(new HorizontalBox(index, token, b, arrow.getWidth(),
			...
			vb.add(new HorizontalBox(index, token, b, arrow.getWidth(),
					TeXConstants.ALIGN_CENTER));
			vb.add(new StrutBox(index, token, 0, sep, 0, 0));
			vb.add(arrow);
			float h = vb.getDepth() + vb.getHeight();

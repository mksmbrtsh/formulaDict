...
public abstract class Box {
	 ...
	 */
	protected Integer foreground = AjLatexMath.foreground;
	 ...
	 */
	protected Integer background = AjLatexMath.background;
	...
	public LinkedList<Box> getChildrens(){
		return children;
	}
	...
	public Box(int index, String token){
		this.index = index;
		this.token = token;
	}
	 ...
	 */
	protected Box(int index, String token, Integer fg, Integer bg) {
		foreground = fg;
		background = bg;
	}
	...
	protected void startDraw(Canvas g2, float x, float y) {
		...
		if (background != null && background != 0) { // draw background

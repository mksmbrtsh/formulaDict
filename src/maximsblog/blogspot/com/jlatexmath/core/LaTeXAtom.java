...
public class LaTeXAtom extends Atom {
	...
	public LaTeXAtom(int index, String token) {
		super(index, token);
	}
	...
	public Box createBox(TeXEnvironment env) {
		...
		HorizontalBox hb = new HorizontalBox(index, token, rat.getLastAtom().createBox(env));
		hb.add(new SpaceAtom(index, token, TeXConstants.UNIT_EM, -0.35f * sc, 0, 0)
				.createBox(env));
		float f = new SpaceAtom(index, token, TeXConstants.UNIT_EX, 0.45f * sc, 0, 0)
				.createBox(env).getWidth();
		float f1 = new SpaceAtom(index, token, TeXConstants.UNIT_EX, 0.5f * sc, 0, 0)
				.createBox(env).getWidth();
		CharBox A = new CharBox(index, token, env.getTeXFont().getChar('A', "mathnormal",
				env.supStyle().getStyle()));
		A.setShift(-f);
		hb.add(A);
		hb.add(new SpaceAtom(index, token, TeXConstants.UNIT_EM, -0.15f * sc, 0, 0)
				.createBox(env));

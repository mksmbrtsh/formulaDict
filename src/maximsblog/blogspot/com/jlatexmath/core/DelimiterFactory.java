...
public class DelimiterFactory {
	...
	public static Box create(int index, String token, SymbolAtom symbol, TeXEnvironment env, int size) {
		if (size > 4)
		...
		if (i <= size && !tf.hasNextLarger(c)) {
			CharBox A = new CharBox(-1, null, tf.getChar('A', "mathnormal", style));
			Box b = create(index, token, symbol.getName(), env,
					size * (A.getHeight() + A.getDepth()));
			return b;
		}

		return new CharBox(symbol.index, symbol.token, c);
	}
	 ...
	 */
	public static Box create(int index, String token, String symbol, TeXEnvironment env, float minHeight) {
		TeXFont tf = env.getTeXFont();
		int style = env.getStyle();
		...
		if (total >= minHeight) { // tall enough character found
			return new CharBox(index, token, c);
			...
			if (ext.hasTop()) { // insert top part
				c = ext.getTop();
				vBox.add(new CharBox(index, token, c));
			}
			...
			c = ext.getRepeat();
			CharBox rep = new CharBox(index, token, c);

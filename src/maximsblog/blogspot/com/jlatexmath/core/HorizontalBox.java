...
public class HorizontalBox extends Box {
	...
	public HorizontalBox(int index, String token, Box b, float w, int alignment) {
		super(index, token);
		if (w != Float.POSITIVE_INFINITY) {
			float rest = w - b.getWidth();
			if (rest > 0) {
				if (alignment == TeXConstants.ALIGN_CENTER
						|| alignment == TeXConstants.ALIGN_NONE) {
					StrutBox s = new StrutBox(index, token, rest / 2, 0, 0, 0);
					add(s);
					add(b);
					add(s);
				} else if (alignment == TeXConstants.ALIGN_LEFT) {
					add(b);
					add(new StrutBox(index, token, rest, 0, 0, 0));
	...
	public HorizontalBox(int index, String token, Box b) {
		super(index, token);
		add(b);
	}

	public HorizontalBox(int index, String token) {
		super(index, token);
		// basic horizontal box
	}

	public HorizontalBox(int index, String token, Integer fg, Integer bg) {
		super(index, token, fg, bg);
	}

	public HorizontalBox cloneBox() {
		HorizontalBox b = new HorizontalBox(index, token, foreground, background);
		b.shift = shift;
	...
	private void recalculate(Box b) {
		...
		depth = Math.max((children.size() == 0 ? Float.NEGATIVE_INFINITY
				: depth), b.depth + b.shift);
	}

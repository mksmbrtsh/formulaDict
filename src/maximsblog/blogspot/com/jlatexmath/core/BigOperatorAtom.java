...
public class BigOperatorAtom extends Atom {
	 ...
	 */
	public BigOperatorAtom(int index, String token, Atom base, Atom under, Atom over) {
		super(index, token);
		this.base = base;
		this.under = under;
		this.over = over;
	 ...
	 *            scripts)
	 */
	public BigOperatorAtom(int index, String token, Atom base, Atom under, Atom over, boolean limits) {
		this(index, token, base, under, over);
		this.limits = limits;
		limitsSet = true;
	}
	...
	public Box createBox(TeXEnvironment env) {
		...
		if ((limitsSet && !limits)
			...
			// superscript
			if (bbase != null) {
				bbase.add(new ScriptsAtom(index, token, base, under, over));
			...
			return new ScriptsAtom(index, token, base, under, over).createBox(env);
		} else {
			if (base instanceof SymbolAtom
				...
				delta = 0;
				y = new HorizontalBox(index, token, base == null ? new StrutBox(index, token, 0, 0, 0, 0)
						: base.createBox(env));
			}
			...
			// over
			if (over != null) {
				vBox.add(new StrutBox(index, token, 0, bigop5, 0, 0));
				x.setShift(delta / 2);
				vBox.add(x);
			...
			// under
			if (under != null) {
				float k = Math.max(tf.getBigOpSpacing2(style),
						tf.getBigOpSpacing4(style) - z.getHeight());
				vBox.add(new StrutBox(index, token, 0, k, 0, 0));
				z.setShift(-delta / 2);
				vBox.add(z);
				vBox.add(new StrutBox(index, token, 0, bigop5, 0, 0));
			}
			...
			if (bbase != null) {
				HorizontalBox hb = new HorizontalBox(index, token, bbase.createBox(env));
				bbase.add(base);
				hb.add(vBox);
				base = Base;
				return hb;
	...
	private static Box changeWidth(Box b, float maxWidth) {
		if (b != null && Math.abs(maxWidth - b.getWidth()) > TeXFormula.PREC)
			return new HorizontalBox(b.index, b.token, b, maxWidth, TeXConstants.ALIGN_CENTER);
		else
			return b;
	}
}

...
public class FractionAtom extends Atom {
	 ...
	 *            the denominator
	 */
	public FractionAtom(int index, String token, Atom num, Atom den) {
		this(index, token, num, den, true);
	}
	 ...
	 */
	public FractionAtom(int index, String token, Atom num, Atom den, boolean rule) {
		this(index, token,num, den, !rule, TeXConstants.UNIT_PIXEL, 0f);
	}
	 ...
	 */
	public FractionAtom(int index, String token, Atom num, Atom den, boolean noDef, int unit, float t)
			throws InvalidUnitException {
		super(index, token);
		// check unit
	 ...
	 */
	public FractionAtom(int index, String token, Atom num, Atom den, boolean rule, int numAlign,
			int denomAlign) {
		this(index, token, num, den, rule);
	 ...
	 *            alignment of the denominator
	 */
	public FractionAtom(int index, String token, Atom num, Atom den, float defFactor, int numAlign,
			int denomAlign) {
		this(index, token, num, den, true, numAlign, denomAlign);
	 ...
	 *            alignment of the denominator
	 */
	public FractionAtom(int index, String token, Atom num, Atom den, int unit, float t, int numAlign,
			int denomAlign) {
		this(index, token, num, den, unit, t);
		this.numAlign = checkAlignment(numAlign);
		this.denomAlign = checkAlignment(denomAlign);
	}
	 ...
	 *            the thickness of the fraction line (in the given unit)
	 */
	public FractionAtom(int index, String token, Atom num, Atom den, int unit, float t) {
		this(index, token, num, den, true, unit, t);
	}
	...
	public Box createBox(TeXEnvironment env) {
		...
		Box num = (numerator == null ? new StrutBox(index, token, 0, 0, 0, 0) : numerator
				.createBox(env.numStyle()));
		Box denom = (denominator == null ? new StrutBox(index, token, 0, 0, 0, 0)
				: denominator.createBox(env.denomStyle()));

		if (num.getWidth() < denom.getWidth())
			num = new HorizontalBox(index, token, num, denom.getWidth(), numAlign);
		else
			denom = new HorizontalBox(index, token, denom, num.getWidth(), denomAlign);
		...
		VerticalBox vBox = new VerticalBox(index, token);
		vBox.add(num);
		...
		if (thickness > 0) { // WITH fraction rule
			...
			// fill vertical box
			vBox.add(new StrutBox(index, token, 0, kern1, 0, 0));
			vBox.add(new HorizontalRule(index, token, thickness, num.getWidth(), 0));
			vBox.add(new StrutBox(index, token, 0, kern2, 0, 0));
			...
			// fill vertical box
			vBox.add(new StrutBox(index, token, 0, kern, 0, 0));
		}
		...
		float f = new SpaceAtom(index, token, TeXConstants.UNIT_EM, 0.12f, 0, 0).createBox(
				env).getWidth();

		return new HorizontalBox(index, token, vBox, vBox.getWidth() + 2 * f,

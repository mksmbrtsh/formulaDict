...
public class NthRoot extends Atom {
	...
	public NthRoot(int index, String token, Atom base, Atom root) {
		super(index, token);
		this.base = base == null ? new EmptyAtom(index, token) : base;
		this.root = root == null ? new EmptyAtom(index, token) : root;
	}
	...
	public Box createBox(TeXEnvironment env) {
		...
		if (style < TeXConstants.STYLE_TEXT)
			clr = tf.getXHeight(style, tf.getChar(sqrtSymbol, style)
					.getFontCode());
		else
			clr = drt;
		...
		HorizontalBox b = new HorizontalBox(index, token, bs);
		b.add(new SpaceAtom(index, token, TeXConstants.UNIT_MU, 1, 0, 0).createBox(env
				.crampStyle()));
		// create root sign
		float totalH = b.getHeight() + b.getDepth();
		Box rootSign = DelimiterFactory.create(index, token, sqrtSymbol, env, totalH + clr
				+ drt);
		...
		OverBar ob = new OverBar(index, token, b, clr, rootSign.getHeight());
		ob.setShift(-(b.getHeight() + clr + drt));
		HorizontalBox squareRoot = new HorizontalBox(index, token, rootSign);
		squareRoot.add(ob);
		...
		if (root == null)
			...
			// negative kern
			Box negativeKern = new SpaceAtom(index, token, TeXConstants.UNIT_MU, -10f, 0, 0)
					.createBox(env);

			// arrange both boxes together with the negative kern
			Box res = new HorizontalBox(index, token);
			float pos = r.getWidth() + negativeKern.getWidth();
			if (pos < 0)
				res.add(new StrutBox(index, token, -pos, 0, 0, 0));

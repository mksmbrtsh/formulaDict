...
public class VRowAtom extends Atom {
	...
	private SpaceAtom raise = new SpaceAtom(index, token, TeXConstants.UNIT_EX, 0, 0, 0);
	protected boolean addInterline = false;

	public VRowAtom(int index, String token) {
		super(index, token);
		// empty
	}

	public VRowAtom(int index, String token, Atom el) {
		super(index, token);
		if (el != null) {
	...
	public void setRaise(int unit, float r) {
		raise = new SpaceAtom(index, token, unit, r, 0, 0);
	}
	...
	public Box createBox(TeXEnvironment env) {
		...
		Box interline = new StrutBox(index, token, 0, env.getInterline(), 0, 0);

...
public class ScriptsAtom extends Atom {
	...
	private final static SpaceAtom SCRIPT_SPACE = new SpaceAtom(-1, null,
	...
	public ScriptsAtom(int index, String token, Atom base, Atom sub, Atom sup) {
		super(index, token);
		this.base = base;
		subscript = sub;
		superscript = sup;
	...
	public ScriptsAtom(int index, String token, Atom base, Atom sub, Atom sup, boolean left) {
		this(index, token, base, sub, sup);
		if (!left)
			align = TeXConstants.ALIGN_RIGHT;
	}

	public Box createBox(TeXEnvironment env) {
		Box b = (base == null ? new StrutBox(index, token, 0, 0, 0, 0) : base.createBox(env));
		Box deltaSymbol = new StrutBox(index, token, 0, 0, 0, 0);
		if (subscript == null && superscript == null)
			...
			if (base.type_limits == TeXConstants.SCRIPT_LIMITS
				...
				return new UnderOverAtom(index, token, new UnderOverAtom(index, token, base, subscript,
						TeXConstants.UNIT_POINT, 0.3f, true, false),
						superscript, TeXConstants.UNIT_POINT, 3.0f, true, true)
						.createBox(env);

			HorizontalBox hor = new HorizontalBox(index, token, b);
			...
			if (base instanceof AccentedAtom) { // special case :
				// accent. This positions superscripts better next to the
				// accent!
				Box box = ((AccentedAtom) base).base
				...
				Box x = new CharBox(index, token, c);

				x.setShift(-(x.getHeight() + x.getDepth()) / 2
						- env.getTeXFont().getAxisHeight(env.getStyle()));
				hor = new HorizontalBox(index, token, x);

				// include delta in width or not?
				delta = c.getItalic();
				deltaSymbol = new SpaceAtom(index, token, TeXConstants.MEDMUSKIP)
						.createBox(env);
				if (delta > TeXFormula.PREC && subscript == null)
					hor.add(new StrutBox(index, token, delta, 0, 0, 0));
			...
			if (superscript == null) { // only subscript
				...
				if (subscript != null && align == TeXConstants.ALIGN_RIGHT) {
					msiz = Math.max(msiz, subscript.createBox(subStyle)
							.getWidth());
				}

				HorizontalBox sup = new HorizontalBox(index, token, x, msiz, align);
				...
				if (subscript == null) { // only superscript
					...
					HorizontalBox sub = new HorizontalBox(index, token, y, msiz, align);
					// add scriptspace (constant value!)
					...
					interSpace = shiftUp - x.getDepth() + shiftDown
							- y.getHeight();
					vBox.add(new StrutBox(index, token, 0, interSpace, 0, 0));
					vBox.add(sub);

...
public class ColorAtom extends Atom implements Row {
	 ...
	 */
	public ColorAtom(int index, String token, Atom atom, Integer bg, Integer i) {
		super(index, token);
		elements = new RowAtom(index, token, atom);
		background = bg;
		color = i;
	}
	 ...
	 */
	public ColorAtom(int index, String token, Integer bg, Integer c, ColorAtom old) {
		super(index, token);
		elements = new RowAtom(index, token, old.elements);
	...
	public static int getColor(String s) {
		if (s != null && s.length() != 0) {
			...
			Integer c = Colors.get(s.toLowerCase());
			if (c != null) {
				return c;
			} else {
				if (s.indexOf('.') != -1) {
					try {
						...
						return Color.rgb((int) g, (int) g, (int) g);
					} catch (NumberFormatException e) {
					}
				}
	...
	private static int convColor(final float c, final float m, final float y,
			final float k) {
		final float kk = 1 - k;
		return Color.rgb((int) (255 * kk * (1 - c)),
				(int) (255 * kk * (1 - m)), (int) (255 * kk * (1 - y)));
	}
}

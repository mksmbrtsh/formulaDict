...
public class RowAtom extends Atom implements Row {
	...
	protected RowAtom(int index, String token) {
		super(index, token);
		// empty
	}

	public RowAtom(int index, String token, Atom el) {
		super(index, token);
		if (el != null) {
	...
	public Atom getLastAtom() {
		...
		return new SpaceAtom(index, token, TeXConstants.UNIT_POINT, 0.0f, 0.0f, 0.0f);
	}
	...
	public Box createBox(TeXEnvironment env) {
		...
		HorizontalBox hBox = new HorizontalBox(index, token, env.getColor(),
				env.getBackground());
		int position = 0;
		env.reset();
		...
		for (ListIterator<Atom> it = elements.listIterator(); it.hasNext();) {
			...
			while (it.hasNext()
					&& atom.getRightType() == TeXConstants.TYPE_ORDINARY
					&& atom.isCharSymbol()) {
				Atom next = it.next();
				position++;
				if (next instanceof CharSymbol
					...
					if (lig == null) {
						...
						atom.changeAtom(new FixedCharAtom(index, token, lig)); // go on with
																	// the
						// ligature
					}
				} else {
			...
			if (it.previousIndex() != 0 && previousAtom != null
				...
				hBox.add(Glue.get(previousAtom.getRightType(),
			...
			if (markAdded
					|| (at instanceof CharAtom && Character
							.isDigit(((CharAtom) at).getCharacter()))) {
			...
			if (Math.abs(kern) > TeXFormula.PREC) {
				hBox.add(new StrutBox(index, token, kern, 0, 0, 0));
			}

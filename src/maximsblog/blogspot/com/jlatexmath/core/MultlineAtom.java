...
public class MultlineAtom extends Atom {
	...
	public MultlineAtom(int index, String token, boolean isPartial, ArrayOfAtoms column, int type) {
		super(index, token);
	...
	public MultlineAtom(int index, String token, ArrayOfAtoms column, int type) {
		this(index, token, false, column, type);
	}

	public Box createBox(TeXEnvironment env) {
		float tw = env.getTextwidth();
		if (tw == Float.POSITIVE_INFINITY || type == GATHERED) {
			return new MatrixAtom(index, token, isPartial, column, "").createBox(env);
		}

		VerticalBox vb = new VerticalBox(index, token);
		Atom at = column.array.get(0).get(0);
		int alignment = type == GATHER ? TeXConstants.ALIGN_CENTER
				: TeXConstants.ALIGN_LEFT;
		if (at.alignment != -1) {
			alignment = at.alignment;
		}
		vb.add(new HorizontalBox(index, token, at.createBox(env), tw, alignment));
		...
		if (column.row > 1) {
			...
			alignment = type == GATHER ? TeXConstants.ALIGN_CENTER

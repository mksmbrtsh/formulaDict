...
public class Glue {
	...
	static {
		GlueSettingsParser parser = new GlueSettingsParser(-1, null);
		glueTypes = parser.getGlueTypes();
		glueTable = parser.createGlueTable();
	}

	public Glue(int index, String token, float space, float stretch, float shrink, String name) {
		this.space = space;
	...
	private Box createBox(TeXEnvironment env) {
		...
		return new GlueBox(index, token, (space / 18.0f) * quad, (stretch / 18.0f) * quad,
				(shrink / 18.0f) * quad);
	}
}

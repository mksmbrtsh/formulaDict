...
public class FencedAtom extends Atom {
	 ...
	 *            the right delimiter
	 */
	public FencedAtom(int index, String token, Atom base, SymbolAtom l, SymbolAtom r) {
		this(index, token, base, l, null, r);
	}

	public FencedAtom(int index, String token, Atom base, SymbolAtom l, List m, SymbolAtom r) {
		super(index, token);
		if (base == null)
			this.base = new RowAtom(index, token); // empty base
		else
			this.base = base;
	...
	public Box createBox(TeXEnvironment env) {
		...
		float shortfall = DELIMITER_SHORTFALL
				* SpaceAtom.getFactor(TeXConstants.UNIT_POINT, env);
		float axis = tf.getAxisHeight(env.getStyle());
		float delta = Math.max(content.getHeight() - axis, content.getDepth()
				+ axis);
		float minHeight = Math.max((delta / 500) * DELIMITER_FACTOR, 2 * delta
				- shortfall);

		// construct box
		HorizontalBox hBox = new HorizontalBox(index, token);

		if (middle != null) {
			for (int i = 0; i < middle.size(); i++) {
				MiddleAtom at = middle.get(i);
				if (at.base instanceof SymbolAtom) {
					Box b = DelimiterFactory.create(index, token,
		...
		// left delimiter
		if (left != null) {
			Box b = DelimiterFactory.create(index, token, left.getName(), env, minHeight);
			center(b, axis);
			hBox.add(b);
		}
		...
		// right delimiter
		if (right != null) {
			Box b = DelimiterFactory.create(index, token, right.getName(), env, minHeight);
			center(b, axis);
			hBox.add(b);
		}

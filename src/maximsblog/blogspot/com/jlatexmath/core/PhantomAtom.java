...
public class PhantomAtom extends Atom implements Row {
	...
	public PhantomAtom(int index, String token, Atom el) {
		super(index, token);
		if (el == null)
			elements = new RowAtom(index, token);
		else
			elements = new RowAtom(index, token, el);
	}

	public PhantomAtom(int index, String token, Atom el, boolean width, boolean height, boolean depth) {
		this(index, token, el);
		w = width;
		h = height;
		d = depth;
	...
	public Box createBox(TeXEnvironment env) {
		...
		return new StrutBox(index, token, (w ? res.getWidth() : 0),
				(h ? res.getHeight() : 0), (d ? res.getDepth() : 0),
				res.getShift());
	}

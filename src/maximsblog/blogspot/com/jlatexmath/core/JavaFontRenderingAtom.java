...
public class JavaFontRenderingAtom extends Atom {
	...
	public JavaFontRenderingAtom(int index, String token, String str, int type) {
		super(index, token);
		this.str = str;
		this.type = type;
	}

	public JavaFontRenderingAtom(int index, String token, String str, TeXFormula.FontInfos fontInfos) {
		this(index, token, str, 0);
		this.fontInfos = fontInfos;
	}

	public Box createBox(TeXEnvironment env) {
		if (fontInfos == null) {
			return new JavaFontRenderingBox(index, token, str, type,
					DefaultTeXFont.getSizeFactor(env.getStyle()));
		} else {
			DefaultTeXFont dtf = (DefaultTeXFont) env.getTeXFont();
			int type = dtf.isIt ? Typeface.ITALIC : Typeface.NORMAL;
			type = type | (dtf.isBold ? Typeface.BOLD : 0);

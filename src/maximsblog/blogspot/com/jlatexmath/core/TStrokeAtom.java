...
public class TStrokeAtom extends Atom {
	...
	public TStrokeAtom(int index, String token, boolean upper) {
		super(index, token);
		this.upper = upper;
	}
	...
	public Box createBox(TeXEnvironment env) {
		...
		CharBox T = new CharBox(index, token, env.getTeXFont().getChar(upper ? 'T' : 't',
				"mathnormal", env.getStyle()));
		CharBox B = new CharBox(index, token, ch);
		Box y;
		if (Math.abs(italic) > TeXFormula.PREC) {
			...
			y = B;
		Box b = new HorizontalBox(index, token, y, T.getWidth(), TeXConstants.ALIGN_CENTER);
		VerticalBox vb = new VerticalBox(index, token);
		vb.add(T);
		vb.add(new StrutBox(index, token, 0, -0.5f * T.getHeight(), 0, 0));
		vb.add(b);
		return vb;
	}
}

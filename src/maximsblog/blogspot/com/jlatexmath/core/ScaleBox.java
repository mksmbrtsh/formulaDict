...
public class ScaleBox extends Box {
	...
	public ScaleBox(int index, String token, Box b, double xscl, double yscl) {
		super(index, token);
		this.box = b;
	...
	public ScaleBox(int index, String token, Box b, float factor) {
		this(index, token, b, (double) factor, (double) factor);
		this.factor = factor;
	}
	...
	public void draw(Canvas g2, float x, float y) {
		...
		if (xscl != 0 && yscl != 0) {
			...
			AjLatexMath.xbase += x + dec;
			AjLatexMath.ybase += y;
			g2.scale((float) xscl, (float) yscl);
			AjLatexMath.xscl *= xscl;
			AjLatexMath.yscl *= yscl;
			box.draw(g2, 0, 0);

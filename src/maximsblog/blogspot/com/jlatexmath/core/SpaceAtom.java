...
public class SpaceAtom extends Atom {
	...
	public SpaceAtom(int index, String token) {
		super(index, token);
		blankSpace = true;
	}

	public SpaceAtom(int index, String token, int type) {
		super(index, token);
		blankSpace = true;
		blankType = type;
	}

	public SpaceAtom(int index, String token, int unit, float width, float height, float depth)
	...
	public SpaceAtom(int index, String token, int widthUnit, float width, int heightUnit, float height,
	...
	public Box createBox(TeXEnvironment env) {
		if (blankSpace) {
			if (blankType == 0)
				return new StrutBox(index, token, env.getSpace(), 0, 0, 0);
			else {
			...
			return new StrutBox(index, token, width * getFactor(wUnit, env), height

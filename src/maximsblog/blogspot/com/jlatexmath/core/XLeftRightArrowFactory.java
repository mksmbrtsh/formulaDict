...
public class XLeftRightArrowFactory {
	...
	public static Box create(boolean left, TeXEnvironment env, float width) {
		...
		Box minus = new SmashedAtom(-1, null, MINUS, "").createBox(env);
		Box kern = new SpaceAtom(-1, null, TeXConstants.UNIT_MU, -4f, 0, 0)
				.createBox(env);
		float mwidth = minus.getWidth() + kern.getWidth();
		swidth += kern.getWidth();
		HorizontalBox hb = new HorizontalBox(-1, null);
		float w;
		...
		hb.add(new SpaceAtom(-1, null, TeXConstants.UNIT_MU, -2f * sf, 0, 0)
				.createBox(env));
		hb.add(new ScaleAtom(-1, null, MINUS, sf, 1).createBox(env));

		if (left) {
			hb.add(0, new SpaceAtom(-1, null, TeXConstants.UNIT_MU, -3.5f, 0, 0)
					.createBox(env));
			hb.add(0, arr);
		} else {
			hb.add(new SpaceAtom(-1, null, TeXConstants.UNIT_MU, -2f * sf - 2f, 0, 0)
					.createBox(env));
			hb.add(arr);
		}
	...
	public static Box create(TeXEnvironment env, float width) {
		...
		if (width < swidth) {
			HorizontalBox hb = new HorizontalBox(index, token, left);
			hb.add(new StrutBox(index, token, -Math.min(swidth - width, left.getWidth()), 0,
					0, 0));
			hb.add(right);
			return hb;
		}
		...
		Box minus = new SmashedAtom(-1, null, MINUS, "").createBox(env);
		Box kern = new SpaceAtom(-1, null, TeXConstants.UNIT_MU, -3.4f, 0, 0)
				.createBox(env);
		float mwidth = minus.getWidth() + kern.getWidth();
		...
		hb.add(new ScaleBox(index, token, minus, (width - swidth - w) / minus.getWidth(), 1));

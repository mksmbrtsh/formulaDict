...
public class CedillaAtom extends Atom {
	...
	public CedillaAtom(int index, String token, Atom base) {
		super(index, token);
		this.base = base;
	}
	...
	public Box createBox(TeXEnvironment env) {
		...
		Box cedilla = new CharBox(index, token, ch);
		Box y;
		...
		Box ce = new HorizontalBox(index, token, y, b.getWidth(), TeXConstants.ALIGN_CENTER);
		float x = 0.4f * SpaceAtom.getFactor(TeXConstants.UNIT_MU, env);
		vb.add(new StrutBox(index, token, 0, -x, 0, 0));
		vb.add(ce);

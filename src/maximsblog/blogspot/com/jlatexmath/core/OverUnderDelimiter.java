...
public class OverUnderDelimiter extends Atom {
	...
	public OverUnderDelimiter(int index, String token, Atom base, Atom script, SymbolAtom s,
			int kernUnit, float kern, boolean over) throws InvalidUnitException {
		super(index, token);
		type = TeXConstants.TYPE_INNER;
		this.base = base;
		this.script = script;
		symbol = s;
		this.kern = new SpaceAtom(index, token,kernUnit, 0, kern, 0);
		this.over = over;
	}
	...
	public Box createBox(TeXEnvironment env) {
		...
		Box del = DelimiterFactory.create(index, token, symbol.getName(), env, b.getWidth());

		Box scriptBox = null;
		if (script != null) {
			scriptBox = script.createBox((over ? env.supStyle() : env
					.subStyle()));
		}
		...
		if (max - b.getWidth() > TeXFormula.PREC) {
			b = new HorizontalBox(index, token, b, max, TeXConstants.ALIGN_CENTER);
		}

		del = new VerticalBox(index, token, del, max, TeXConstants.ALIGN_CENTER);
		if (scriptBox != null && max - scriptBox.getWidth() > TeXFormula.PREC) {
			scriptBox = new HorizontalBox(index, token, scriptBox, max,
					TeXConstants.ALIGN_CENTER);
		}

		return new OverUnderBox(index, token, b, del, scriptBox, kern.createBox(env)
				.getHeight(), over);
	}

...
public class AccentedAtom extends Atom {
	...
	public AccentedAtom(int index, String token, Atom base, Atom accent)
	...
	public AccentedAtom(int index, String token, Atom base, Atom accent, boolean changeSize)
			throws InvalidSymbolTypeException {
		this(index, token, base, accent);
	 ...
	 */
	public AccentedAtom(int index, String token, Atom base, String accentName)
	 ...
	 *             if the symbol is not defined as an accent ('acc')
	 */
	public AccentedAtom(int index, String token, Atom base, TeXFormula acc)
	...
	public Box createBox(TeXEnvironment env) {
		...
		float delta = acc ? ec : Math.min(b.getHeight(),
		...
		Box cb = new CharBox(index, token, ch);
		if (acc)
			cb = accent.createBox(changeSize ? env.subStyle() : env);

		if (Math.abs(italic) > TeXFormula.PREC) {
			y = new HorizontalBox(index, token, new StrutBox(index, token, -italic, 0, 0, 0));
			y.add(cb);
		} else
			y = cb;
		...
		if (diff < 0)
			b = new HorizontalBox(index, token, b, y.getWidth(), TeXConstants.ALIGN_CENTER);
		vBox.add(y);

		// kern
		vBox.add(new StrutBox(index, token, 0, changeSize ? -delta : -b.getHeight(), 0, 0));
		// base
		vBox.add(b);
		...
		if (diff < 0) {
			HorizontalBox hb = new HorizontalBox(index, token, new StrutBox(index, token, diff, 0, 0, 0));
			hb.add(vBox);
			hb.setWidth(u);
			return hb;
		}
